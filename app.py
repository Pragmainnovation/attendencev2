...
def attendance_page():
    ...
    if st.button('Submit Attendance', disabled=submit_disabled):
        ...
        if is_near:
            # Query the database for all records for this employee, sort by name, then datetime
            conn = get_data_db_connection()
            c = conn.cursor()
            c.execute('''SELECT type, date, time FROM attendance WHERE name = ? ORDER BY name, date, time''', (recognized_name,))
            rows = c.fetchall()
            conn.close()
            last_type = None
