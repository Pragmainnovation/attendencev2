...
def attendance_page():
    ...
    if st.button('Submit Attendance', disabled=submit_disabled):
        ...
        if is_near:
            # Query the database for all records for this employee, sort by datetime
            conn = get_data_db_connection()
            c = conn.cursor()
            ...
            if rows:
                ...
                for t, d, tm in rows:
                    try:
                        ...
                        continue
                records = [r for r in records if r[1] is not pd.NaT and r[1] is not None]
                if records:
                    records.sort(key=lambda x: x[1])
                    last_type = records[-1][0]
                    if last_type == 'Check In':
                        # Find the last check-in datetime
                        checkin_records = [r for r in records if r[0] == 'Check In']
                        if checkin_records:
                            last_checkin_dt = checkin_records[-1][1]
            now = datetime.now()
            now_time = now.time()
            hour_value = ""
            if last_type == 'Check In':
